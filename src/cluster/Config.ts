...
export default {
  ...
  handshake: assert<boolean>(
    ...
    `${CLUSTER_HANDSHAKE}`.toLowerCase() === 'true' || parseInt(CLUSTER_HANDSHAKE) === 1,
  ...
  rollingRestarts: assert<boolean>(
    ...
    `${CLUSTER_ROLLING_RESTARTS}`.toLowerCase() === 'true' || parseInt(CLUSTER_ROLLING_RESTARTS) === 1,
