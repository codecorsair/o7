...
const regex =
  /([a-zA-Z0-9\- ]+[a-zA-Z\-](?: [0-9]+(?!\/))?)(?:(?:\s*)([0-5]+(?:\/[0-5]+){1,})?)(?:(?:\s*)([0-5]+(?:\/[0-5]+){1,})?)/;

export async function getCapitalResponseList(
  searchText: string
): Promise<string[] | null> {
  ...
  const nameMulti: string[] = name.split(' ');
...
export async function getResponseCapital(
  searchText: string,
  isMobile: boolean
) {
  const parsedArgs = searchText.toLowerCase().match(regex);
  ...
  // Too many results
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `I don't found any item with this word ... Did you mean :` +
          '\n' +
          listItem
      );
  ...
  embed.addFields([
    {
      name: `Manufacture`,
      value: `${bp.type} Skills **${skillLevels.join(
        '/'
      )}**\nMaterial Efficiency **${numeral(mod.material).format(
        '0%'
      )}**\nTime Efficiency **${numeral(mod.time).format('0%')}**`,
      inline: true
    }
  ]);
  embed.addFields([
    {
      name: `Market`,
      value: `Accounting Skills **${accountingLevels.join(
        '/'
      )}**\nBroker's Fee **${numeral(accountingRates.brokersRate).format(
        '0.0%'
      )}**\nTransaction Tax **${numeral(accountingRates.taxRate).format(
        '0%'
      )}**`,
      inline: true
    }
  ...
  if (hasAny(bp, componentKeys)) {
    const description = await printKeys(
      bp,
      componentKeys,
      mod.material,
      total,
  ...
  embed.addFields([
    {
      name: 'Production',
      value: printProduction(bp, mod.time, isMobile),
      inline: false
    }
  ]);
...
function printCosts(
  ...
  const sellOrderLow =
    (latestPrice && latestPrice.lowest_sell * bp.productionCount) || 0;
  const sellOrderMed =
    (latestPrice && latestPrice.sell * bp.productionCount) || 0;
  ...
  let result = '```\n';
  result += alignText(
    `Cost to build${isMobile ? '\n' : ''}`,
    `${numeral(total.cost).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `Blueprint Cost${isMobile ? '\n' : ''}`,
    `low ${numeral(bpPrice.lowest_sell).format(
      '0[.]0a'
    )} ISK | median ${numeral(bpPrice.sell).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `Market sell${isMobile ? '\n' : ''}`,
    `low ${numeral(sellOrderLow).format('0[.]0a')} ISK | median ${numeral(
      sellOrderMed
    ).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `Broker's Fee${isMobile ? '\n' : ''}`,
    `low ${numeral(brokersFeeLow).format('0[.]0a')} ISK | median ${numeral(
      brokersFeeMed
    ).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `Transaction Tax${isMobile ? '\n' : ''}`,
    `low ${numeral(taxFeeLow).format('0[.]0a')} ISK | median ${numeral(
      taxFeeMed
    ).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `Profit margin${isMobile ? '\n' : ''}`,
    `low ${numeral(sellOrderLow - total.cost - brokersFeeAndTaxLow).format(
      '0[.]0a'
    )} ISK | median ${numeral(
      sellOrderMed - total.cost - brokersFeeAndTaxMed
    ).format('0[.]0a')} ISK\n`,
    isMobile
  );
  result += alignText(
    `(If buying BP)${isMobile ? '\n' : ''}`,
    `low ${numeral(
      sellOrderLow - (total.cost + bpPrice.lowest_sell) - brokersFeeAndTaxLow
    ).format('0[.]0a')} ISK | median ${numeral(
      sellOrderMed - (total.cost + bpPrice.sell) - brokersFeeAndTaxMed
    ).format('0[.]0a')} ISK\n`,
    isMobile
  );
  return result + '```';
...
async function printKeys(
  ...
  for (const key of keys) {
    ...
    groupCost += cost;
    result += `${isMobile ? '\n' : ''} [${numeral(price.buy).format(
      '0[.]0a'
    )} ISK > ${numeral(cost).format('0[.]0a')} ISK]\n`;
  }
  result += `\n${alignText(
    'Total Cost',
    `${numeral(groupCost).format('0[.]0a')} ISK`,
    isMobile
  )}`;
...
function printProduction(bp: any, timeMod: number, isMobile: boolean) {
  ...
  result += alignText(
    'Manufacturing cost',
    numeral(bp.productionCost).format('0[.]0a') + ' ISK\n',
    isMobile
  );
  result += alignText(
    'Manufacturing time ',
    printDuration(
      moment.duration(Math.ceil(bp.productionTime * 1000 * timeMod))
    ) + '\n',
    isMobile
  );
...
function printDuration(duration: any) {
  return `${duration.days() > 0 ? duration.days() + 'd ' : ''}${duration
    .hours()
    .toLocaleString(undefined, { minimumIntegerDigits: 2 })}:${duration
    .minutes()
    .toLocaleString(undefined, { minimumIntegerDigits: 2 })}:${duration
    .seconds()
...
function alignText(key: string, value: any, isMobile: boolean) {
  ...
  return (
    capitalize(key) +
    stringValue.padStart(column - key.length + stringValue.length, ' ')
  );
}
...
function skillModifier(skillLevels: number[]) {
  ...
  return {
    material:
      baseMaterialEfficiencyRate -
      ((stdLvl && stdLvl * stdMatPerLvl) || 0) -
      ((advLvl && advLvl * advMatPerLvl) || 0) -
      ((expLvl && expLvl * expMatPerLvl) || 0),
    time:
      baseTimeRate -
      ((stdLvl > 0 && timeModPerLvl[stdLvl - 1]) || 0) -
...
function accountingSkillModifier(accountingSkillLevels: number[]) {
  ...
  return {
    brokersRate:
      baseBrokersRate -
      ((stdLvl && stdLvl * brokerPerLvl) || 0) -
      ((advLvl && advLvl * brokerPerLvl) || 0) -
      ((expLvl && expLvl * brokerPerLvl) || 0),
    taxRate:
      baseTaxRate -
      ((stdLvl > 0 && stdTaxModPerLvl[stdLvl - 1]) || 0) -
...
function normalizeSkills(skillsToCheck: string) {
  const skillLevels = (skillsToCheck &&
    skillsToCheck.split('/').map((s: string) => parseInt(s))) || [0, 0, 0];
  const stdLvl = skillLevels[0] || 0;
  const advLvl = skillLevels[1] || 0;
  const expLvl = skillLevels[2] || 0;
